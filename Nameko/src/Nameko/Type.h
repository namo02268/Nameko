...
namespace TypeUtils {
	...
	struct remove_type<T, list<Ts...>> {
		using type = list<Ts...>;  // デフォルトでは何も削除しない
	};

	// Tがリストの先頭にある場合、それを削除し、残りのリストを返す
	template <typename T, typename... Ts>
	struct remove_type<T, list<T, Ts...>> {
		using type = list<Ts...>;
	};

	// Tがリストの先頭以外にある場合、再帰的に処理する
	template <typename T, typename U, typename... Ts>
	struct remove_type<T, list<U, Ts...>> {
		using type = typename concat<
			list<U>,
			typename remove_type<T, list<Ts...>>::type
		>::type;
	};
}
